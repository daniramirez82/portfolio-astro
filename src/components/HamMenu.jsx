...
const HamMenu = ({ cates }) => {
  ...
  return (
    <div className="flex flex-col justify-end">
      ...
      <div
        className={`absolute transition-all duration-500 right-0  z-30 rounded-md drop-shadow-md border bg-white border-amber-300 px-7 p-5 mr-4 mt-7 ${
          isOpen ? "opacity-100 top-12" : "opacity-0 top-0"
        }`}
      >
        <ul className="flex flex-col w-fit space-y-5">
          {cates.map((i) => (
            <li className="w-fit self-end " key={i}>
              {i}
            </li>
          ))}
        </ul>
